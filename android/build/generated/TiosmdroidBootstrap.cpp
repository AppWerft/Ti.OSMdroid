...
#define TAG "ti.osmdroid"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tiosmdroid_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tiosmdroid.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TiosmdroidBindings::lookupGeneratedInit(
...
static void Tiosmdroid_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tiosmdroid_getBinding)->GetFunction());
}

static void Tiosmdroid_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiosmdroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiosmdroidBinding = {
	"ti.osmdroid",
	Tiosmdroid_init,
	Tiosmdroid_dispose
};
...
Java_ti_osmdroid_TiosmdroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.osmdroid", &TiosmdroidBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiosmdroidBindings::lookupGeneratedInit));
}
